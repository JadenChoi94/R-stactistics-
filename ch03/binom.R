...
(px<-dbinom(x, size=n, prob=p))# 0일떄 1일때 ~ 6일때 
'''
d : 확률
p : 누적확률
...
ex=sum(x*px) #기댓값 Expectation value 
ex2<-sum(x^2 * px)
...
plot(x, nd, type='l', xlab='x', ylab='P(X=x)', lwd=2, col='red',
     main='평균170, 표준편차 6인 정규분포곡선')
pnorm(mu, mean=mu, sd=sigma)
pnorm(158, mean=mu, sd=sigma)
pnorm(180, mean=mu, sd=sigma)-pnorm(160, mean=mu, sd=sigma) #160부터 180까지인 사람이 90.4%다
